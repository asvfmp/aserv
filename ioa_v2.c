...
#define DBG	'D'+'B'+'G'
    ...
    int time_acc = 10;							// Частота обновления сигнала ACC и напряжения батареи
    int old_acc = 0;							// сохраненное значение сигнала ACC
...
void setAttr(int fd, unsigned int baud)
{
    ...
    toptions.c_cflag &= ~HUPCL;			// Сброс HUPCL
...
int displayResult(int fd, char otv[])
{
	...
	printf("Количество символов ответа:	%zu\n",sz);
    }
    for (int i=0; i<=(sz-2); i++) otv[i]=buf[i]; 
}

//------------ Читаем команды для контроллера от сервера и передаем в контроллер ------------------------//
void read_stat(int signo)
{
    ...
    if (signo == SIGUSR1)
    {
	if((fp=fopen(msg_f1, "r"))==NULL) 			// команда от сервера
	{
    ...
    {
	if((fp=fopen(msg_f2, "r"))==NULL) 			// команда от сервера записи на флэшку
	{
	    printf("Не удается открыть файл.\n");
    	    exit(1);
	}
...
// Вычисляем хэш статуса сигнала светодиодов
    ...
    I_O_ard(comd,div);							// отправили команду контроллеру
...
// ------------------------------ чтение/запись контроллера ---------------------------------------------//
void I_O_ard (int SIG,char *SIG_arg)
{
    FILE *fp_acc;							// указатель на файл статуса ACC
    FILE *fp_vb;							// указатель на файл статуса батареи
    char ret[50]={0};
    char tm[80]={0};							/* Строка для посылки команды в контроллер */
	...
	case ACC:
		...
		res = displayResult(fd,ret);				// Читаем ответ контроллера
		if (ret[0] > '0' && old_acc == 0)			// Если изменился сигнал - пишем в файл
		{
		    old_acc = 1;					// запоминаем на будущее
		    fp_acc=fopen(acc_f,"w");				// открываем файл
		    fprintf(fp_acc,"ON");				// пишем результат
		    fclose(fp_acc);
...
//using namespace std;
    ...
    sigset_t   set;
    sigemptyset(&set);
    sigaddset(&set, SIGUSR1);						// Сигнал от основного сервера
    sigaddset(&set, SIGUSR2);						// Сигнал от сервера записи на флэшку
    sigaddset(&set, SIGTERM);						// Сигнал завершения от сервера
    sigaddset(&set, SIGALRM);						// Сигнал для возврата из ожидания сигнала для опроса контроллера
    act.sa_mask = set;
	...
	int opt = 0;
    struct globalArgs_t {						// Формирование аргументов строки
...
//    globalArgs.numInputFiles = 0;
    ...
    opt = getopt( argc, argv, optString );				// Разбор аргументов строки опций
	while( opt != -1 ) {
    	    switch( opt ) {
        	case 'd':
...
//    globalArgs.numInputFiles = argc - optind;
    ...
    dbg = globalArgs.noDebug;						// Отладочный режим
    fileName = globalArgs.portContr;					// Файл устройства контроллера FT232RL
...
//    baud = getBaudRate(argc, argv);
    ...
    while(wrk)
	{
		...
		if (shoot_sig == SIGALRM)
		    {
		    if (dbg) printf("Полученный сигнал:		SIGALRM\n");				// Приняли сигнал тревоги от таймера
		    I_O_ard(ACC,0);									// Получили значение ACC
		    I_O_ard(VB,0);									// Получили напряжение батареи
		    alarm(time_acc);									// Взвели таймер
		    }
		else if (shoot_sig == SIGTERM)
		    {
		    if (dbg) printf("Полученный сигнал:		SIGTERM\n");				// Приняли сигнал от сервера на завершение работы
		    wrk=false;
		    }
		else
		    {
		    if (dbg) printf("Полученный сигнал SIGUSR:	%u\n",shoot_sig);				// Приняли запрос от сервера на предоставление данных
		    read_stat(shoot_sig);
		    }
		if (dbg) {printf("Вышли из обработчика\n"); printf("----------------------------------------------\n");}
	}
